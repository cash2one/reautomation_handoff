...
class IAPSession():
    ...
    def getPrompt(self, timeout=-1, idleTimeout=60):
        ...
        patList = ['User: $',                           'assword:',                        '\[confirm\] $',
                                 ...
                                 '\(y/n\)',                           '--More--',                          "%s%s" %(self.myDevice.get("mac"),prompt),
                                 ':~\$',                           winpexpect.TIMEOUT,                      winpexpect.EOF,
                                 'Login : $',                       'config\) #',   'autoboot',         'eth1 is up']
        ...
        while True:
            ...
            if index == 0+i:
                ## Login: $
                self.connectId.send("%s\r" %self.username)
                try:
                    self.connectId.expect('assword: ',10)
                except winpexpect.TIMEOUT:
                    pass
                else:
                    self.connectId.send("%s\r" %self.password)
                    out = self.connectId.before + self.connectId.after
                    self.output += out
            elif index == 1+i:
                ## assword: $
                ...
                ## \[confirm\] $
                self.connectId.send("\r")
            elif index == 3+i:
                ## \[confirm yes/no\]:
                self.connectId.send('yes')
                out = self.connectId.before + self.connectId.after
                self.output += out
            elif index == 4+i:
                ## \[yes/no\]:
...
#                    log.debug('idle timer %s' %idleTimer, level=const.LEVEL4)
                    ...
                    if timeoutAttemptCount < 5:
                        self.connectId.send("\r\r")
                        timeoutAttemptCount += 1
                    else:
                        raise winpexpect.TIMEOUT('connection timed out')
            elif index == 11+i:
                ...
                self.connectId.send("%s\r" %self.myDevice.get("consoleUsername"))
                try:
                    self.connectId.expect('assword :', 10)
                except winpexpect.TIMEOUT:
                    pass
                else:
                    self.connectId.send(self.myDevice.get('consolePassword'))
                    self.connectId.send("\r\r")
                    self.connectId.send("\r\r")
                    #self.connectId.send("%s\r" %self.username)
                    #self.connectId.send("test123")
                    #self.connectId.send("%s\r" %self.password)
                    #if self.connectId.expect("User:"):
                    #self.connectId.send("%s\r" % self.username)
                    # self.connectId.send("test123")
                    #self.connectId.send("admin\r")
                    out = self.connectId.before + self.connectId.after
                    self.output += out
            elif index == 13+i:
                self.connectId.send("end")
            elif index == 14+i:
                pass
            elif index == 15+i:
                self.connectId.send("\r")
