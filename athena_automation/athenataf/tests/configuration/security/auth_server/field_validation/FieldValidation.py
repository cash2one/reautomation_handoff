...
class FieldValidation(ConfigurationTest):
	...
	def test_ath_12059_field_validation_for_radius_server(self):
		...
		security.create_authentication_server(name=conf.auth_server_name_value, ip=conf.auth_server_ip_valid, sharedkey=conf.internal_server_password,retypekey=conf.internal_server_password,mask = conf.invalid_radius_server_ip,vlan = conf.invalid_radius_server_drp_vlan_4094)
	...
	def test_ath_12058_delete_the_created_radius_server(self):
		...
		security = self.LeftPanel.go_to_security()
		security.create_auth_server.click()
		security.create_authentication_server(name=conf.auth_rad1, ip=conf.auth_rad1_ip, sharedkey=conf.internal_server_password,retypekey=conf.internal_server_password, vlan=conf.auth_retry_count)
		security.save_auth_server()
		security.click_edit_auth_server()
		security.create_authentication_server(ip=conf.auth_ip, sharedkey=conf.auth_shared_key1,retypekey=conf.auth_shared_key1,rfc=True,actport=conf.auth_acct_port,timeout=conf.vlan,retrycount= conf.auth_retry_count,authport = conf.auth_port_value,nas_ip=conf.auth_nas_ip,nas_identifier_value=conf.auth_nas_identifier,coa_port=conf.auth_port_valid, vlan=conf.auth_retry_count)
		security.save_auth_server()
		security.create_auth_server.click()
		security.create_authentication_server(name=conf.auth_radius_name1, ip=conf.auth_radius_ip1, sharedkey=conf.auth_radius_shared_key1,retypekey=conf.auth_radius_shared_key1,rfc=True,nas_ip=conf.nas_ip,nas_identifier_value=conf.auth_nsi_identifier_new, vlan=conf.auth_retry_count)
		security.save_auth_server()
		self.take_s2_snapshot()
		security.delete_authentication_server()
		security.delete_authentication_server()
