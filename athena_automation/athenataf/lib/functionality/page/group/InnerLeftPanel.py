...
class InnerLeftPanel(WebPage):
	...
	def delete_multiple_groups(self):
		'''
		deletes groups
		'''
		if self.assert_group1_and_group2():
			if self.assert_sample_group_with_vc_present():
				manage_group_page = self.manage_group()
		...
		for i in range(3,11):
			...
			string = string+str(i)
			elem = None
			exec("elem = manage_group.%s"% string)
			if elem:
				elem.click()
				time.sleep(3)
				manage_group.delete.click()
				manage_group.buy_time()
				manage_group.reboot_ok_button.click()
		manage_group.close.click()	
		self.group_close_icon.click()
		
	def assert_device_present_in_default_group(self):
		logger.debug('InnerLeftPanel: Clicking on plus(+) icon to expand the group')
		if not self.expand_group_icon :
			raise AssertionError("InnerLeftPanel: plus(+) icon	not found for default group")
		else:
			self.expand_group_icon.click()
			time.sleep(4)
			logger.debug('InnerLeftPanel: Checking virtual controller if present in default group')
			time.sleep(10)
			if not self.default_group_swarm_name:
				raise AssertionError("InnerLeftPanel: virtual controller not found in default group")
				
	def assert_mygroup_0_with_vc_present(self):
		if self.mygroup_0_name_with_vc:
			return True
		else :
			return False
	
	def assert_mygroup_0_without_vc_present(self):
		if self.mygroup_0_name_without_vc:
			return True
		else :
			return False
			
	def assert_mygroup_2_with_vc_present(self):
		if self.mygroup2_name_with_vc:
			return True
		else :
			return False
	
	def assert_mygroup_2_without_vc_present(self):
		if self.mygroup2_name_without_vc:
			return True
		else :
			return False
