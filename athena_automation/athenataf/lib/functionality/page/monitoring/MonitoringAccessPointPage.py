...
class MonitoringAccessPointPage(WebPage):
	...
	def verify_device_mac_address(self, mac_addr = None):
		'''
		Asserting mac address field
		'''
		mac_address_1 = self.mac_address.get_label_text()
		if not mac_address_1 == mac_addr:
			raise AssertionError("AccessPoint : MAC address are not matching ")