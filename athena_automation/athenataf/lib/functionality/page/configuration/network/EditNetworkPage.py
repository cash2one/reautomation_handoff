...
class EditNetworkPage(WebPage):
    ...
    def create_new_vlan_rule(self):
        ...
        self.vlan_id_2.set(self.config.config_vars.vlan_id_4093)
    ...
    def change_bw_to_vlan(self):
        ...
        self.rule_type_1.set(self.config.config_vars.access_rule_vlan_assignment)
        logger.debug('EditNetworkPage : Setting vlan id')
        self.vlan_id_2.set(self.config.config_vars.vlan_id_1)
        logger.debug('EditNetworkPage : Clicking on Save')
        self.save_rule_button.click()
        time.sleep(5)
        logger.debug('EditNetworkPage : Clicking on Save button')
    ...
    def change_bw_to_captive_portal(self):
        ...
        self.bw_rule_edit_button.click()
        logger.debug('EditNetworkPage : Changing Rule type to captive portal')
        self.rule_type_1.set(self.config.config_vars.access_rule_captive_portal)
        logger.debug('EditNetworkPage : Clicking on Save')
        self.save_rule_button.click()
        time.sleep(5)
        logger.debug('EditNetworkPage : Checking bandwidth contract rule is present or not')
    ...
    def assert_new_rule_created(self, service, destination):
        ...
        if not self.new_service_1.get_selected() == service:
        ...
        if not self.new_destination_1.get_selected() == destination:
    ...
    def edit_calea_rule(self):
        ...
        if not self.rule_type_1.get_selected() == self.config.config_vars.rule_type_calea:
...
#         time.sleep(5)
    ...
    def assert_authentication_server_changes(self):
        ...
        self.security_accordion.click()
        time.sleep(20)
        if self.authentication_server.get_selected() == self.config.config_vars.auth_server_value:
    ...
    def change_auth_server_2_alone_to_external_server(self):
        logger.debug('EditNetworkPage : Clicking on Security accordion')
        self.security_accordion.click()
        time.sleep(20)
    ...
    def edit_security_configurations(self):
        ...
        if not self.authentication_server.selected=='InternalServer':
    ...
    def check_security_configurations1(self):
        ...
        if not self.mac_authentication.selected=='Enabled':
    ...
    def check_security_configurations_for_external_radius_server(self):
        ...
        if not self.authentication_server.selected=='authradius':
    ...
    def check_security_configurations_for_external_radius_server2(self):
        ...
        if not self.open_uppercase_support.selected=='Enabled':
        ...
        if not self.authentication_server_2.selected=='testradius_2':
    ...
    def set_static_vlan(self):
        ...
        self.vlanid.set(self.config.config_vars.edit_Vlan_Id_new)
    ...
    def select_internalserver_2(self):
        self.buy_time()
        self.buy_time()
        logger.debug("EditNetworkPage : Selecting  InternalServer for Authentication Server feild ")
        self.authentication_server_2.set(self.config.config_vars.edit_Authentication_server)
        self.buy_time()
    ...
    def checking_key_managementpassphrase_format(self):
        ...
        if not self.security_key_management.selected=='Both(WPA-2 & WPA)':
        ...
        if not self.blacklisting.selected=='Enabled':
        ...
        if not self.passphrase_format.selected=='8-63 chars':
    ...
    def check_static_wep_Tx_key(self):
        ...
        if not self.security_key_management.selected=='Static WEP':
        ...
        if not self.wep_key_index.selected=='1':
        ...
        if not self.wep_key_size.selected=='128-bit':
    ...
    def check_static_network_changes(self):
        ...
        if not self.wep_key_index.selected=='4':
    ...
    def check_wpa_blacklisting_enable(self):
        ...
        if not self.security_key_management.selected=='WPA Personal':
    ...
    def check_wpa_blacklisting_disable(self):
        ...
        if not self.blacklisting.selected=='Disabled':
        ...
        if not self.passphrase_format.selected=='64 hexadecimal chars':
    ...
    def assert_auth_server_settings(self,accounting_enable=False,acc_interval=False,balancing=False,auth_survivability=False,okc=False,blacklisting=False,max_auth_failures=False, termination_disable=False, termination_enable=False,blacklisting_disable=False):
        '''
        ...
        if accounting_enable:
            ...
            if not self.accounting_dropdown.selected == self.config.config_vars.enabled:
        ...
        if balancing:
            ...
            if not self.load_balancing_dropdown.selected == self.config.config_vars.enabled:
        ...
        if auth_survivability:
            ...
            if not self.auth_survivability.selected == self.config.config_vars.enabled:
        ...
        if termination_disable:
            ...
            if not self.security_termination.selected == self.config.config_vars.disabled:
        ...
        if termination_enable:
            ...
            if not self.security_termination.selected == self.config.config_vars.enabled:
        ...
        if blacklisting:
            ...
            if not self.blacklisting.selected == self.config.config_vars.enabled:
        ...
        if blacklisting_disable:
            logger.debug("EditNetworkPage :Check if blacklisting is disabled.")
            if not self.blacklisting.selected == self.config.config_vars.disabled:
                raise AssertionError("blacklisting is enabled.Traceback: %s " %traceback.format_exc())
    ...
    def configure_auth_server_settings(self,accounting_disable=False,balancing_enable=False,acc_interval=False,balancing=False,auth_survivability=False,roaming=False,okc=False,termination_disable=False,accounting_enable=False,termination_enable=False,auth_survivability_enable=False):
...
#                 self.account_interval.set(self.config.config_vars.edit_accounting_interval)
    ...
    def checking_network_wpa_2_personal(self):
        ...
        if not self.security_key_management.selected=='WPA-2 Personal':
    ...
    def check_enable_optimization_feilds(self):
        ...
        if not self.multicastratetransmission.selected=='Enabled':
        ...
        if not self.dynamicmulticast.selected=='Enabled':
    ...
    def check_personal_employee_network_with_dynamic_multicast_optimization(self):    
        ...
        if not self.accounting_dropdown.selected=='Enabled':
        ...
        if not self.load_balancing_dropdown.selected=='Enabled':
        ...
        if not self.open_uppercase_support.selected=='Disabled':
            raise AssertionError("Uppercase support feild is disabled .Traceback: %s " %traceback.format_exc())
    ...
    def check_advance_settings_and_reauth_interval(self):
        ...
        if not self.reauth_unit.selected=='hrs.':
    ...
    def assert_reauth_interval_session_key_with_advanced_settings_changes(self):
        ...
        logger.debug('EditNetworkPage : Clicking on security accordion')
        self.security_accordion.click()
        if not self.reauth_interval.get() == '3':
            raise AssertionError("Reauth interval is not set to 3.Traceback: %s " %traceback.format_exc())
    ...
    def assert_auth_survivability_cache_timeout_enable(self):
        '''
            Assert authenticate survivability and cache timeout
        '''
        if not self.auth_survivability:
            raise AssertionError("Authentication Survivability is not visible.Traceback: %s " %traceback.format_exc())
        if not self.cache_timeout:
            raise AssertionError("Cache timeout is not visible.Traceback: %s " %traceback.format_exc())
        logger.debug('EditNetworkPage : Clicking on Network link')
        self.networks.click()
        time.sleep(5)
    ...
    def asserting_modified_created_network(self):
        ...
        self.security_accordion.click()
        if self.splash_page_type.get_selected() == self.config.config_vars.internal_acknowledge:
    ...
    def set_enterprise_both_wpa_2_and_wpa_enterprise_voice(self):
        ...
        # if self.account_interval.is_enabled() :
    ...
    def asserting_enterprise_both_wpa_2_and_wpa_enterprise_voice(self):
        ...
        # logger.debug("EditNetworkPage: Asserting the Accounting Internal field")
        # if not self.account_interval.is_visible():
        #         raise AssertionError("EditNetworkPage: Accounting Internal field is not set to '0' ")
    ...
    def edit_enterprise_both_wpa2_and_wpa_enterprise_employee_network_with_inactive_timeout(self):
        ...
        #self.authentication_server.set(conf.edit_auth_server_name)
    ...
    def assert_static_wep_and_wep_key(self):
        ...
        '''
        self.browser.assert_drop_down_value(self.band,self.config.config_vars.band_value,'Band value is not set to 2.4 GHz')
        self.click_security_accordion()
        self.browser.assert_drop_down_value(self.security_key_management,self.config.config_vars.static_wep_value,'Security Managament is not set to Static wep')
    ...
    def assert_open_voice_network_with_accounting_interval_uppercase_supported(self):
        ...
        self.click_security_accordion()
        logger.debug("EditNetworkPage: Asserting SecurityPage Fields ")
        self.browser.assert_drop_down_value(self.authentication_server_2,conf.auth_server_value_2, "AuthenticationServer 2 not set'")
        self.browser.assert_drop_down_value(self.accounting_dropdown,conf.enabled_option, "Accounting field is not set to 'Enable'")
        self.browser.assert_text(self.account_interval,conf.new_accounting_interval,'Accounting interval not set to "60"','value')
        self.browser.assert_drop_down_value(self.open_uppercase_support,conf.enabled_option, "Uppercase Support field is not set to 'Enable'")
